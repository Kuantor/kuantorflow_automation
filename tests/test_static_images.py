...
# The two `hi_there.jpg` cases that were here are gone with the picture
# (kuantorflow#199). They measured the gate card's artwork against the card
# it filled -- its width against `CARD_CONTENT_PX`, and that `.gate-image`
# carried no `max-width`, which is what had made it a stamp above two
# full-width controls (#361). There is no gate page, no `.gate-image` rule
# and no `hi_there.jpg`, so both assertions now describe nothing. #361's
# rule itself is unaffected and is still enforced above, for every image
# that is left.
